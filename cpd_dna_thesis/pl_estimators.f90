...
!uses Lucy PL estimators to gather raw data from simulation results
!--------------------------------------------------------------------------------
  SUBROUTINE PL_ESTIMATORS(pkt_count,incident_spec_irr)
    ...
    open(10,file='./data_out/depths_JAQUES.dat',status='replace')
    depth_val=0.0d0
    do k=1,nzg
      do j=1,nyg
        do i=1,nxg
    ...
    open(10,file='./data_out/depths2.dat',status='replace')
