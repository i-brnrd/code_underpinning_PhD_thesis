...
if (nwl.gt.1) then
!  do i=1,nwl MOVED INTO THE INIT MODULE
...
!  enddo
  ...
  do i=1,nwl
    write(8,*) l(i),u_s_all(1:nlayer,i), u_a_all(1:nlayer,i),g_all(i)
  enddo
  close(8)

!SSR
   !fname_incident_irradiation='./spectra/ssr_300w_16s.txt'
...
!*****Release photon from source *******************************
  ...
  !assumes wavelength won't change
        ...
        !POSITIONALLY DEPENDANT OPTICAL PROPERTY:
        do while(inside)
          albedo=0.d0
           do i =1,nlayer
